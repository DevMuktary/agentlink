...
import { 
  FileBadge, History, AlertTriangle, 
...
export default function VninSlipClient({ service }: { service: ServiceData | null }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const customReference = `DASH-VNIN-${Date.now()}`;

    try {
      const res = await fetch('/api/v1/identity/vnin-slip', {
        method: 'POST',
      ...
      if (!res.ok || data.status === false) {
        throw new Error(data.error || 'Failed to generate VNIN slip.');
      }
  ...
  const handleDownloadPDF = () => {
    ...
    // Create a temporary link element to trigger the base64 download
    const linkSource = `data:application/pdf;base64,${pdfData}`;
    const downloadLink = document.createElement('a');
    const fileName = `VNIN_Slip_${successData?.nin || 'Generated'}.pdf`;
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-300">
            <div className="p-6 sm:p-8">
              ...
              <div className="space-y-4 text-sm text-slate-600 dark:text-slate-400 font-medium leading-relaxed">
                <p>
                  You are about to generate a Virtual NIN (VNIN) Slip for the provided National Identity Number.
                </p>
                <p className="text-indigo-700 dark:text-indigo-400 font-bold bg-indigo-50 dark:bg-indigo-500/10 p-3 rounded-xl border border-indigo-100 dark:border-indigo-500/20">
                  Your wallet will only be charged if the slip is generated successfully.
                </p>
                <p>
                  Once generated, the PDF slip will be available for immediate download. You can also re-download it anytime from your transaction history at no extra cost.
                </p>
              </div>
            </div>
        ...
        <div className="mb-8">
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
            <div className="p-2.5 bg-indigo-50 dark:bg-indigo-500/10 rounded-xl text-indigo-600 dark:text-indigo-400">
              <FileBadge size={26} strokeWidth={2.5} />
            </div>
            VNIN Slip Generation
          </h1>
          <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">
            Generate and download official Virtual NIN slips instantly.
          </p>
        </div>
          ...
          <div className="mb-8 p-4 bg-orange-50 dark:bg-orange-500/10 border border-orange-200 dark:border-orange-500/20 rounded-2xl flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-orange-700 dark:text-orange-400/80 mt-1 font-medium">
                VNIN Slip generation is currently undergoing scheduled maintenance by our provider. Please check back later.
              </p>
            </div>
          </div>
        )}
              ...
              {successData && pdfData ? (
                <div className="text-center py-6 animate-in zoom-in duration-300 relative z-10">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto mb-8 text-sm font-medium">
                    The VNIN Slip has been processed and is ready for download.
                  </p>
                  
                  {/* BIG DOWNLOAD BUTTON */}
                  <button 
                    onClick={handleDownloadPDF}
                    className="mx-auto mb-8 py-4 px-8 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-2xl transition-all shadow-lg shadow-indigo-600/20 flex items-center justify-center gap-3 hover:-translate-y-1 active:scale-[0.98]"
                  >
                ...
                <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                  ...
                  {service && (
                    <div className="p-4 bg-indigo-50/50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-900/30 rounded-xl flex items-center justify-between shadow-sm">
                  ...
                  {isServiceDown && (
                    <div className="p-3.5 bg-orange-50 dark:bg-orange-500/10 rounded-xl text-sm text-orange-700 dark:text-orange-400 font-bold flex items-center gap-2 border border-orange-200 dark:border-orange-500/20 shadow-sm">
                      ...
                      VNIN Slip service is currently disabled.
                    </div>
                  )}

                  <button 
                    type="submit" 
                    disabled={isServiceDown || loading || nin.length !== 11}
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Lost your slip?</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed mb-6 font-medium">
                  If you misplaced a previously generated slip, you can safely re-download it from your history log without being charged again.
                </p>
                
                <Link 
                  href="/dashboard/history/vnin"
                ...
                >
                  Go to VNIN History <ArrowRight size={16} className="text-slate-400 group-hover:translate-x-1 group-hover:text-indigo-500 transition-all" />
                </Link>
              </div>
            </div>
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                <div className="p-1 bg-indigo-100 dark:bg-indigo-500/20 rounded-md text-indigo-600 dark:text-indigo-400">
                  <Info size={14} />
                </div>
              ...
              <ul className="space-y-4 text-xs font-medium text-slate-600 dark:text-slate-400">
                ...
                <li className="flex items-start gap-2.5">
                  <div className="w-1.5 h-1.5 rounded-full bg-indigo-400 dark:bg-indigo-500 mt-1.5 shrink-0" />
                  Generated PDFs are securely stored in your history for future retrieval.
                </li>
              </ul>
            </div>
