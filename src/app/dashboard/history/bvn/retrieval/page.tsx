...
import { 
  ...
  Monitor, Code, ChevronLeft, ChevronRight, Database, Eye, Info, XCircle as XCircleIcon, Copy, Check
} from 'lucide-react';
...
export default function BvnRetrievalHistoryPage() {
  ...
  const fetchHistory = async () => {
    try {
      ...
      const logs = res.data.filter((r: any) => 
        r.serviceType === 'BVN_RETRIEVAL_PHONE' || r.serviceType === 'BVN_RETRIEVAL_CRM'
      );
      setRequests(logs);
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(r => 
        ...
        r.requestData?.ticket_id?.includes(q) || 
  ...
  const handleCopyBvn = (bvn: string) => {
    if (!bvn) return;
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-5 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
            <Database className="text-fuchsia-500" size={24} /> BVN Retrieval
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 font-medium">
            Monitor manually processed BVN retrieval updates.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          ...
          <div className="flex p-1 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl w-full sm:w-auto overflow-x-auto">
            ...
            <button 
              ...
              className={`flex-1 sm:flex-none px-4 py-2 text-sm font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 whitespace-nowrap ${sourceFilter === 'DASHBOARD' ? 'bg-white dark:bg-slate-800 text-fuchsia-600 dark:text-fuchsia-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-fuchsia-600 dark:hover:text-fuchsia-400'}`}
            >
          ...
          <div className="relative w-full sm:w-auto">
            ...
            <select 
              ...
              className="w-full sm:w-48 pl-10 pr-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-fuchsia-500 text-base sm:text-sm font-medium text-slate-700 dark:text-slate-200 appearance-none cursor-pointer shadow-sm"
            >
              <option value="ALL">All Categories</option>
              <option value="BVN_RETRIEVAL_PHONE">By Phone Number</option>
              <option value="BVN_RETRIEVAL_CRM">By CRM</option>
            </select>
          </div>
          ...
          <div className="relative w-full sm:w-auto">
            ...
            <input 
              ...
              className="w-full sm:w-64 pl-10 pr-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-fuchsia-500 text-base sm:text-sm font-medium text-slate-900 dark:text-white placeholder:text-slate-400 shadow-sm"
            />
          </div>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {paginatedRequests.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-16 text-center text-slate-500 dark:text-slate-400 font-medium">
                    <div className="flex flex-col items-center justify-center">
                      <Database className="w-10 h-10 text-slate-300 dark:text-slate-600 mb-3" />
                      No BVN Retrieval records found.
                    </div>
                  </td>
                </tr>
              ) : (
                paginatedRequests.map((item) => {
                  ...
                  const isCRM = item.serviceType === 'BVN_RETRIEVAL_CRM';
                    ...
                    <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <div className="font-bold text-slate-900 dark:text-white">
                          {isCRM ? 'By CRM' : 'By Phone Number'}
                        </div>
                        <div className="font-mono text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">
                          {isCRM ? `Ticket: ${item.requestData?.ticket_id}` : `Phone: ${item.requestData?.phone_number}`}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <button 
                          ...
                          className="inline-flex items-center justify-center bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 hover:border-fuchsia-500 hover:text-fuchsia-600 dark:hover:text-fuchsia-400 text-slate-700 dark:text-slate-300 px-3 py-1.5 rounded-lg font-bold transition-all shadow-sm"
                        >
        ...
        <div className="fixed inset-0 bg-slate-900/60 dark:bg-black/80 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col border border-slate-200 dark:border-slate-800">
            ...
            <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950 shrink-0">
              <h3 className="font-bold text-lg text-slate-900 dark:text-white">Retrieval Details</h3>
            ...
            <div className="p-6 space-y-6 overflow-y-auto">
              ...
              {/* --- STATUS: COMPLETED (THE COPY BVN LOGIC) --- */}
              {selectedItem.status === 'COMPLETED' && (
                <div className="space-y-4">
                   <div className="p-4 bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/20 rounded-2xl flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-green-800 dark:text-green-300 text-sm">Record Retrieved Successfully</h4>
                    </div>
                  </div>

                  <div className="p-5 border-2 border-fuchsia-100 dark:border-fuchsia-900/30 rounded-2xl bg-fuchsia-50/50 dark:bg-fuchsia-500/5 text-center">
                      <span className="block text-xs text-fuchsia-600 dark:text-fuchsia-400 uppercase font-bold tracking-wider mb-2">
                        Retrieved BVN
                      </span>
                      <div className="flex items-center justify-center gap-3">
                        <span className="text-3xl font-mono font-extrabold text-slate-900 dark:text-white tracking-wider">
                           {selectedItem.responseData?.bvn || selectedItem.responseData?.number || 'Unavailable'}
                        </span>
                        {(selectedItem.responseData?.bvn || selectedItem.responseData?.number) && (
                           <button 
                             onClick={() => handleCopyBvn(selectedItem.responseData?.bvn || selectedItem.responseData?.number)}
              ...
              {selectedItem.status === 'FAILED' && (
                <div className="p-4 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-2xl flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-red-800 dark:text-red-300 text-sm">Retrieval Failed</h4>
              ...
              <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                <h4 className="font-bold text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider">Search Parameters</h4>
                    ...
                    {selectedItem.serviceType === 'BVN_RETRIEVAL_PHONE' ? (
                      <>
                        <div className="p-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/30">
                            <span className="block text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold mb-1">Target Phone</span>
                        ...
                        <div className="p-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/30">
                            ...
                            <span className="font-bold text-slate-900 dark:text-white capitalize">{selectedItem.requestData?.full_name}</span>
                        </div>
                      </>
                    ) : (
                      <>
                        <div className="p-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/30">
                            <span className="block text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold mb-1">Agent Code</span>
                            <span className="font-bold font-mono text-slate-900 dark:text-white">{selectedItem.requestData?.agent_code}</span>
                        </div>
                        <div className="p-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/30">
                            <span className="block text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold mb-1">Ticket ID</span>
                            <span className="font-bold font-mono text-slate-900 dark:text-white">{selectedItem.requestData?.ticket_id}</span>
                        </div>
                      </>
                    )}
                </div>
