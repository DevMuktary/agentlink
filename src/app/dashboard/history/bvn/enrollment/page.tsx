...
import { 
  ...
  Monitor, Code, ChevronLeft, ChevronRight, UserPlus, Eye, Info, XCircle as XCircleIcon
} from 'lucide-react';

export default function BvnEnrollmentHistoryPage() {
  ...
  const fetchHistory = async () => {
    try {
      const res = await axios.get('/api/user/requests?type=ANDROID_BVN_ENROLLMENT'); 
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-5 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
            <UserPlus className="text-violet-500" size={24} /> BVN Enrollment
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 font-medium">
            Monitor your agent enrollment requests.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          ...
          <div className="flex p-1 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl w-full sm:w-auto overflow-x-auto">
            ...
            <button 
              ...
              className={`flex-1 sm:flex-none px-4 py-2 text-sm font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 whitespace-nowrap ${sourceFilter === 'DASHBOARD' ? 'bg-white dark:bg-slate-800 text-violet-600 dark:text-violet-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-violet-600 dark:hover:text-violet-400'}`}
            >
          ...
          <div className="relative w-full sm:w-auto">
            ...
            <input 
              type="text" 
              placeholder="Search Name, BVN or Ref..." 
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full sm:w-64 pl-10 pr-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500 text-base sm:text-sm font-medium text-slate-900 dark:text-white placeholder:text-slate-400 shadow-sm"
            />
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50 dark:bg-slate-950/50 border-b border-slate-200 dark:border-slate-800">
              <tr>
                ...
                <th className="px-6 py-4 font-bold text-slate-600 dark:text-slate-300">Applicant Details</th>
                    ...
                    <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-right">
                        <button 
                          ...
                          className="inline-flex items-center justify-center bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 hover:border-violet-500 hover:text-violet-600 dark:hover:text-violet-400 text-slate-700 dark:text-slate-300 px-3 py-1.5 rounded-lg font-bold transition-all shadow-sm"
                        >
        ...
        <div className="fixed inset-0 bg-slate-900/60 dark:bg-black/80 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col border border-slate-200 dark:border-slate-800">
            ...
            <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950 shrink-0">
              <h3 className="font-bold text-lg text-slate-900 dark:text-white">Enrollment Details</h3>
            ...
            <div className="p-6 space-y-6 overflow-y-auto">
              ...
              {selectedItem.status === 'COMPLETED' && (
                <div className="space-y-4">
                   <div className="flex items-center gap-2 text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-500/10 p-3 rounded-xl border border-green-200 dark:border-green-500/20">
                      ...
                      <span className="font-bold text-sm">Enrollment Approved!</span>
                   </div>
                   <p className="text-sm text-slate-600 dark:text-slate-400 font-medium">
                      Check the registered email <span className="font-bold text-slate-900 dark:text-white">({selectedItem.requestData?.email})</span> for your NIBSS login credentials.
                   </p>
                </div>
              )}
              ...
              {selectedItem.status === 'FAILED' && (
                 <div className="p-5 bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400 border border-red-200 dark:border-red-500/20 rounded-2xl flex flex-col gap-2">
                    <div className="flex items-center gap-2 font-bold text-base">
                        <XCircleIcon className="w-5 h-5 shrink-0" /> Enrollment Rejected
                    </div>
                    <p className="text-sm pl-7 text-red-600/90 dark:text-red-400/90 font-medium">
                        Reason: <span className="font-bold">{selectedItem.adminNote || 'Declined by Admin'}</span>
                    </p>
                 </div>
              )}
              ...
              {selectedItem.status === 'PROCESSING' && (
                 <div className="p-5 bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-500/20 rounded-2xl flex flex-col items-center justify-center gap-3 text-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm opacity-80 mt-1">This application is under review. Please expect feedback within 5 working days.</p>
                    </div>
                 </div>
              )}
              ...
              <div className="space-y-4 pt-6 border-t border-slate-100 dark:border-slate-800">
                <p className="font-bold text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider">Application Data</p>
                <div className="grid grid-cols-2 gap-3 text-sm">
                    <div className="p-4 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/30">
                        <span className="block text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">Applicant Name</span>
                        <span className="font-bold text-slate-900 dark:text-white capitalize">{selectedItem.requestData?.first_name} {selectedItem.requestData?.last_name}</span>
                    </div>
                     <div className="p-4 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/30">
                        <span className="block text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">Target BVN</span>
                    ...
                    <div className="col-span-2 p-4 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/30">
                        <span className="block text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">Registered Email</span>
                        <span className="font-bold text-slate-900 dark:text-white">{selectedItem.requestData?.email}</span>
                    </div>
                </div>
              </div>
