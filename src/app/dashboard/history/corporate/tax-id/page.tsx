...
import { 
  ...
  Monitor, Code, ChevronLeft, ChevronRight, Landmark, Eye, Info, XCircle as XCircleIcon, Copy, Check, Download
} from 'lucide-react';
  ...
  const fetchHistory = async () => {
    try {
      ...
      const logs = res.data.filter((r: any) => 
        r.serviceType === 'TAX_ID_INDIVIDUAL' || r.serviceType === 'TAX_ID_NON_INDIVIDUAL'
      );
      setRequests(logs);
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-5 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
            <Landmark className="text-blue-500" size={24} /> Tax ID Requests
          </h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 font-medium">
            Monitor manually processed TIN generations.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          ...
          <div className="relative w-full sm:w-auto">
            ...
            <select 
              ...
              className="w-full sm:w-48 pl-10 pr-4 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-base sm:text-sm font-medium text-slate-700 dark:text-slate-200 appearance-none cursor-pointer shadow-sm"
            >
              <option value="ALL">All Categories</option>
              <option value="TAX_ID_INDIVIDUAL">Individual</option>
              <option value="TAX_ID_NON_INDIVIDUAL">Corporate</option>
            </select>
          </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {paginatedRequests.length === 0 ? (
                ...
                paginatedRequests.map((item) => {
                  ...
                  const isCorporate = item.serviceType === 'TAX_ID_NON_INDIVIDUAL';
                  const targetName = isCorporate 
                    ...
                    <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        ...
                        <div className="font-mono text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">
                          {isCorporate ? `RC: ${item.requestData?.rc_number}` : `NIN: ${item.requestData?.nin}`}
                        </div>
                      </td>
        ...
        <div className="fixed inset-0 bg-slate-900/60 dark:bg-black/80 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col border border-slate-200 dark:border-slate-800">
            ...
            <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950 shrink-0">
              <h3 className="font-bold text-lg text-slate-900 dark:text-white">Tax ID Details</h3>
              <button onClick={() => { setSelectedItem(null); setCopied(false); }} className="text-slate-400 hover:text-red-500 transition-colors p-1">
            ...
            <div className="p-6 space-y-6 overflow-y-auto">
              ...
              {/* --- STATUS: COMPLETED (THE BIG COPY BOX) --- */}
              {selectedItem.status === 'COMPLETED' && (
                <div className="space-y-4">
                  <div className="p-4 bg-green-50 dark:bg-green-500/10 border border-green-200 dark:border-green-500/20 rounded-2xl flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-green-800 dark:text-green-300 text-sm">Tax ID Generated</h4>
                    </div>
                  </div>

                  <div className="p-5 border-2 border-blue-100 dark:border-blue-900/30 rounded-2xl bg-blue-50/50 dark:bg-blue-500/5 text-center shadow-sm">
                      <span className="block text-xs text-blue-600 dark:text-blue-400 uppercase font-bold tracking-wider mb-2">
                        Official Tax ID (TIN)
                      </span>
                      <div className="flex items-center justify-center gap-3">
                        <span className="text-3xl font-mono font-extrabold text-slate-900 dark:text-white tracking-wider">
                           {selectedItem.responseData?.tax_id || selectedItem.responseData?.tin || selectedItem.responseData?.number || 'N/A'}
                        </span>
                        {(selectedItem.responseData?.tax_id || selectedItem.responseData?.tin || selectedItem.responseData?.number) && (
                           <button 
                             onClick={() => handleCopyTaxId(selectedItem.responseData?.tax_id || selectedItem.responseData?.tin || selectedItem.responseData?.number)}
                             className="p-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-all text-slate-600 dark:text-slate-300 active:scale-90 shadow-sm"
                             title="Copy Tax ID"
                           >
                  ...
                  {/* OPTIONAL SLIP DOWNLOAD */}
                  {(selectedItem.responseData?.slip_url || selectedItem.responseData?.resultUrl || selectedItem.responseData?.url) && (
                    <a 
                      href={selectedItem.responseData?.slip_url || selectedItem.responseData?.resultUrl || selectedItem.responseData?.url} 
                      target="_blank" rel="noreferrer"
                      className="flex items-center justify-center gap-2 p-4 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all text-slate-700 dark:text-slate-300 font-bold text-sm shadow-sm"
                    >
                      <Download className="w-4 h-4" /> Download Official Slip
                    </a>
                  )}
                </div>
              )}
              ...
              {selectedItem.status === 'FAILED' && (
                <div className="p-4 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-2xl flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-red-800 dark:text-red-300 text-sm">Generation Failed</h4>
                    <p className="text-xs text-red-700 dark:text-red-400/90 font-medium mt-1">Reason: {selectedItem.adminNote || 'Record not found. Your wallet has been refunded.'}</p>
                  </div>
                </div>
              )}
              ...
              {selectedItem.status === 'PROCESSING' && (
                <div className="p-4 bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 rounded-2xl flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-blue-700 dark:text-blue-400/90 font-medium mt-0.5">This request is awaiting admin processing.</p>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                <h4 className="font-bold text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider">Submitted Details</h4>
                    ...
                    {selectedItem.serviceType === 'TAX_ID_INDIVIDUAL' ? (
                      <>
                        <div className="col-span-2 p-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/30">
                            <span className="block text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold mb-1">Target Name</span>
                            <span className="font-bold text-slate-900 dark:text-white capitalize">{selectedItem.requestData?.first_name} {selectedItem.requestData?.middle_name} {selectedItem.requestData?.surname}</span>
                        </div>
                        <div className="p-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/30">
                            <span className="block text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold mb-1">Target NIN</span>
                            <span className="font-bold font-mono text-slate-900 dark:text-white">{selectedItem.requestData?.nin}</span>
                        </div>
                        <div className="p-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/30">
                            <span className="block text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold mb-1">Date of Birth</span>
                            <span className="font-bold font-mono text-slate-900 dark:text-white">{selectedItem.requestData?.dob}</span>
                        </div>
                      </>
                    ) : (
                      <>
                        <div className="col-span-2 p-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/30">
                            <span className="block text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold mb-1">Business Name</span>
                            <span className="font-bold text-slate-900 dark:text-white capitalize">{selectedItem.requestData?.business_name}</span>
                        </div>
                        <div className="col-span-2 p-3 border border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/30">
                            <span className="block text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold mb-1">RC / BN Number</span>
                            <span className="font-bold font-mono text-slate-900 dark:text-white uppercase">{selectedItem.requestData?.rc_number}</span>
                        </div>
                      </>
                    )}
                </div>
