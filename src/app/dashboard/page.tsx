...
import { 
  ...
  Activity, ArrowRight, FileText, FileDigit, Users, CheckCircle2
} from 'lucide-react';
...
export default function DashboardHome() {
  ...
  const services = [
    { title: "NIN Verification", icon: ShieldCheck, href: "/dashboard/services/nin-verification", color: "bg-blue-600", desc: "Logs for NIN & Phone Lookup" },
    { title: "VNIN Slip", icon: FileText, href: "/dashboard/services/vnin", color: "bg-indigo-600", desc: "Download Premium Slips" },
    { title: "IPE Clearance", icon: ShieldCheck, href: "/dashboard/services/nin/ipe-clearance", color: "bg-red-600", desc: "Track Clearance Status" },
    { title: "NIN Personalization", icon: Users, href: "/dashboard/services/nin/personalization", color: "bg-pink-600", desc: "Personalization History" },
    { title: "BVN Services", icon: CheckCircle2, href: "/dashboard/services/bvn", color: "bg-cyan-600", desc: "Verification & Mod Logs" },
    { title: "JAMB Services", icon: GraduationCap, href: "/dashboard/services/jamb", color: "bg-green-600", desc: "UTME/DE Registration Logs" },
    { title: "Airtime & Data", icon: Wifi, href: "/dashboard/services/utilities", color: "bg-emerald-600", desc: "VTU Transaction History" },
    { title: "CAC Registration", icon: Building2, href: "/dashboard/services/cac", color: "bg-purple-600", desc: "Business Name Logs" },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-gradient-to-br from-slate-900 to-slate-800 dark:from-blue-900 dark:to-slate-900 rounded-2xl p-6 sm:p-8 text-white shadow-xl relative overflow-hidden group">
          <div className="relative z-10">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href="/dashboard/wallet" className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-2.5 rounded-lg text-sm font-semibold shadow-lg transition-all">
                + Fund Wallet
              </Link>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col justify-between">
          ...
          <Link href="/dashboard/developers" className="mt-4 flex items-center justify-between text-sm text-blue-600 dark:text-blue-400 hover:text-blue-700 font-medium group">
            View API Keys <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
          </Link>
        </div>
      </div>
      <div>
        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-5 flex items-center">Service History</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {services.map((service, index) => (
            <Link key={index} href={service.href} className="group bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-100 dark:border-gray-700 hover:shadow-lg hover:border-blue-100 dark:hover:border-blue-900 transition-all duration-300">
              ...
              <h4 className="text-lg font-bold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{service.title}</h4>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 leading-relaxed">{service.desc}</p>
            </Link>
          ))}
        </div>
      </div>
