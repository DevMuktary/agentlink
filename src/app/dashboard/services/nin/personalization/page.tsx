  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        ...
        <button onClick={fetchHistory} className="p-2 bg-gray-100 dark:bg-gray-800 rounded-full hover:bg-gray-200 transition"><RefreshCw className="w-5 h-5 text-gray-500" /></button>
      </div>
                ...
                requests.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                    ...
                    <td className="px-6 py-4 text-gray-500 dark:text-gray-400 truncate max-w-xs">
                      {item.responseData?.message || item.adminNote || 'Processing...'}
                    </td>
                  </tr>
                ))
              )}
