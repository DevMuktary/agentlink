...
import { 
  ...
  AlertTriangle, User, Layers, Globe, Code, Copy, Check
} from 'lucide-react';
...
export default function AdminNinValidationQueue() {
  ...
  const getTypeBadge = (type: string) => {
      if (type.includes('VNIN')) return <span className="bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 px-2 py-1 rounded text-[9px] font-bold uppercase border border-purple-200 dark:border-purple-800/50">vNIN Validation</span>;
      if (type.includes('NO_RECORD')) return <span className="bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 px-2 py-1 rounded text-[9px] font-bold uppercase border border-orange-200 dark:border-orange-800/50">No Record</span>;
      if (type.includes('UPDATE')) return <span className="bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 px-2 py-1 rounded text-[9px] font-bold uppercase border border-blue-200 dark:border-blue-800/50">Record Update</span>;
      return <span className="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 px-2 py-1 rounded text-[9px] uppercase border border-gray-200 dark:border-gray-700">Validation</span>;
  };
            ...
            <div className="flex items-center gap-3">
                ...
                {strValue !== 'N/A' && (
                    <button 
                        ...
                        className="text-slate-300 dark:text-slate-600 hover:text-purple-500 dark:hover:text-purple-400 transition-colors bg-white dark:bg-slate-800 p-1.5 rounded-md shadow-sm border border-slate-200 dark:border-slate-700"
    ...
    <div className="space-y-6 animate-in fade-in pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div>
            ...
            <Search className="w-8 h-8 text-purple-600 dark:text-purple-400" /> NIN Validations
            </h1>
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1 font-medium">Manage Validation, No Record Found & VNIN Issues.</p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        <div className="overflow-x-auto">
            ...
            <thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-700">
                ...
                <th className="px-6 py-4 font-bold">NIN / Identifier</th>
                ...
                {filteredRequests.length === 0 ? (
                    <tr>
                        <td colSpan={6} className="px-6 py-16 text-center text-slate-500 dark:text-slate-400 font-medium">
                            No validation requests found for the selected channel.
                        </td>
                    </tr>
                ) : (
                    filteredRequests.map((item) => {
                        ...
                        <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                            ...
                            <td className="px-6 py-4">
                                {item.status === 'COMPLETED' && <span className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider flex w-fit items-center gap-1 border border-green-200 dark:border-green-800/50"><CheckCircle2 size={12}/> Success</span>}
                                ...
                                {item.status === 'PROCESSING' && <span className="bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider flex w-fit items-center gap-1 border border-purple-200 dark:border-purple-800/50"><RefreshCw size={12} className="animate-spin"/> Pending</span>}
                            </td>
            ...
            <div className="flex justify-between items-start border-b border-slate-100 dark:border-slate-800 pb-4 sticky top-0 bg-white dark:bg-slate-900 z-10 pt-2">
              <div>
                 <h3 className="font-black text-2xl text-slate-900 dark:text-white tracking-tight mb-2">
                     {selectedItem.status === 'PROCESSING' ? 'Process Validation Request' : 'Validation Record'}
                 </h3>
                ...
                <div className="space-y-6 h-full overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    <div className="bg-purple-50 dark:bg-purple-900/10 p-5 rounded-2xl border border-purple-100 dark:border-purple-800/30">
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-xl border border-purple-100 dark:border-purple-800/50 p-2">
                            {Object.entries(selectedItem.requestData || {}).map(([key, value]) => {
                                ...
                                // Clean up the key string for display
                    ...
                    <details className="group">
                        <summary className="flex items-center gap-2 cursor-pointer text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors bg-slate-50 dark:bg-slate-800 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
                            <Code size={14} /> Toggle Raw JSON Payload
                        </summary>
                        ...
                        <>
                            ...
                            <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 flex-1 flex flex-col justify-center">
                                <h4 className="font-black text-slate-900 dark:text-white mb-5 flex items-center gap-2 border-b border-slate-200 dark:border-slate-700 pb-3 uppercase tracking-wide text-sm">
                                    <CheckCircle2 className="text-green-600 dark:text-green-500" size={20} /> Mark As Completed
                                </h4>
