...
import { 
  ...
  Globe, Code, Copy, Check, Search, X
} from 'lucide-react';
...
export default function AdminCacQueue() {
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(r => 
          ...
          r.requestData?.business_details?.proposed_name_2?.toLowerCase().includes(q) ||
  ...
  const handleActionSubmit = async () => {
    ...
    try {
      ...
      const note = actionType === 'REJECT' ? rejectionReason : (adminNote || 'Approved');
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      ...
      case 'PROCESSING': return 'bg-orange-100 text-orange-800 border-orange-200 dark:bg-orange-900/30 dark:text-orange-400 dark:border-orange-800/50';
  ...
  // Bulk Formatting
  const formatBusinessDetails = (item: any) => {
    const d = item.requestData?.business_details || {};
    return `Name 1: ${d.proposed_name_1 || 'N/A'}; Name 2: ${d.proposed_name_2 || 'N/A'}; Nature: ${d.nature_of_business || 'N/A'}; Desc: ${d.description || 'N/A'}; Address: ${d.address || 'N/A'}, ${d.lga || 'N/A'}, ${d.state || 'N/A'};`;
  };

  const formatProprietorDetails = (item: any) => {
    const d = item.requestData?.proprietor_details || {};
    return `Name: ${d.firstname || ''} ${d.middle_name || ''} ${d.surname || ''}; NIN: ${d.nin || 'N/A'}; Phone: ${d.phone || 'N/A'}; Email: ${d.email || 'N/A'}; Address: ${d.address || 'N/A'}, ${d.lga || 'N/A'}, ${d.state || 'N/A'};`;
  };
    ...
    <div className="space-y-6 pb-20 max-w-[90rem] mx-auto animate-in fade-in duration-500">
      ...
      <div className="flex flex-col lg:flex-row gap-4 justify-between bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
        <div>
           <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
             <Building2 className="w-7 h-7 text-orange-600 dark:text-orange-400" /> CAC Registrations
           </h1>
           <p className="text-gray-500 text-sm mt-1 font-medium">Manage corporate business registration requests.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-center">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input 
              type="text"
              placeholder="Search Business, Agent, Ref..."
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              className="pl-9 pr-4 py-2.5 w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-sm font-medium text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500 outline-none transition-all placeholder:text-slate-400"
            />
          </div>
          ...
          <select 
            ...
            className="w-full sm:w-auto bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2.5 text-sm font-bold text-slate-700 dark:text-slate-300 outline-none focus:ring-2 focus:ring-orange-500 cursor-pointer shadow-sm"
          >
          ...
          <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
            <thead className="bg-slate-50 dark:bg-slate-950/50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider whitespace-nowrap">Proposed Name 1</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider whitespace-nowrap">Nature of Business</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
              {filteredRequests.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-slate-500 dark:text-slate-400 font-medium">No CAC requests found.</td>
                </tr>
              ) : (
                filteredRequests.map((req) => (
                  <tr key={req.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-900 dark:text-white font-bold uppercase whitespace-nowrap max-w-[200px] truncate" title={req.requestData?.business_details?.proposed_name_1}>
                      {req.requestData?.business_details?.proposed_name_1 || 'N/A'}
                    </td>
                    <td className="px-6 py-4 text-xs text-slate-600 dark:text-slate-400 font-medium uppercase whitespace-nowrap max-w-[150px] truncate">
                      {req.requestData?.business_details?.nature_of_business || 'N/A'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                       <div className="flex justify-end items-center gap-2">
                         ...
                         <button 
                           ...
                           className="p-1.5 text-slate-500 dark:text-slate-400 hover:text-orange-600 dark:hover:text-orange-400 hover:bg-orange-50 dark:hover:bg-orange-900/30 rounded-full transition-colors"
                           title="View Details"
                         >
                           ...
                           <div className="flex gap-1.5">
                             {req.status === 'PENDING' && (
                               <button 
                                 ...
                                 className="text-xs bg-orange-50 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 font-bold px-3 py-1.5 rounded border border-orange-200 dark:border-orange-800/50 hover:bg-orange-100 dark:hover:bg-orange-900/50 transition-colors shadow-sm"
                               >
                                 Process
                               </button>
                             )}
                         ...
                         {req.status === 'COMPLETED' && req.responseData?.certificate_url && (
                           <a 
                             href={req.responseData.certificate_url} 
                             target="_blank" 
      ...
      {viewReq && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 dark:bg-black/80 backdrop-blur-sm p-4 overflow-y-auto animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-4xl relative my-8 border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200">
            ...
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 p-6 bg-slate-50 dark:bg-slate-950/50 rounded-t-3xl">
              <div>
                 <h3 className="text-xl font-black text-slate-900 dark:text-white flex items-center gap-2">
                   <Building2 className="w-5 h-5 text-orange-500" /> Application Details
                 </h3>
            ...
            <div className="p-6 max-h-[70vh] overflow-y-auto space-y-6 custom-scrollbar">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Business Data */}
                  <div className="border border-orange-200 dark:border-orange-800/50 rounded-xl overflow-hidden shadow-sm shadow-orange-100/50 dark:shadow-none h-fit flex flex-col">
                    <div className="bg-orange-50 dark:bg-orange-900/10 p-3 border-b border-orange-200 dark:border-orange-800/50 flex justify-between items-center">
                        <span className="font-bold text-orange-900 dark:text-orange-500 text-sm">Business Info</span>
                        <button 
                            onClick={() => copyText(formatBusinessDetails(viewReq), 'business_details')}
                            className="text-[10px] bg-white dark:bg-slate-800 border border-orange-200 dark:border-orange-700 text-orange-700 dark:text-orange-500 font-bold px-2.5 py-1.5 rounded shadow-sm hover:bg-orange-50 dark:hover:bg-orange-900/30 flex items-center gap-1 uppercase tracking-wider transition-colors"
                        >
                            {copiedId === 'business_details' ? <Check size={12} className="text-green-500" /> : <Copy size={12} />}
                            Copy Info
                        </button>
                    </div>
                    <div className="p-4 space-y-0 bg-white dark:bg-slate-900 text-sm">
                        <DisplayRow label="Proposed Name 1" value={viewReq.requestData?.business_details?.proposed_name_1} />
                        <DisplayRow label="Proposed Name 2" value={viewReq.requestData?.business_details?.proposed_name_2} />
                        <DisplayRow label="Nature of Business" value={viewReq.requestData?.business_details?.nature_of_business} />
                        <DisplayRow label="Description" value={viewReq.requestData?.business_details?.description} />
                        <DisplayRow label="Address" value={viewReq.requestData?.business_details?.address} />
                        <DisplayRow label="State" value={viewReq.requestData?.business_details?.state} />
                        <DisplayRow label="LGA" value={viewReq.requestData?.business_details?.lga} />
                    </div>
                  </div>

                  {/* Proprietor Data */}
                  <div className="border border-indigo-100 dark:border-indigo-800/30 rounded-xl overflow-hidden shadow-sm h-fit flex flex-col">
                    <div className="bg-indigo-50 dark:bg-indigo-900/10 p-3 border-b border-indigo-100 dark:border-indigo-800/30 flex justify-between items-center">
                        <span className="font-bold text-indigo-900 dark:text-indigo-400 text-sm">Proprietor Info</span>
                        <button 
                            onClick={() => copyText(formatProprietorDetails(viewReq), 'proprietor_details')}
                        ...
                        >
                            {copiedId === 'proprietor_details' ? <Check size={12} className="text-green-500" /> : <Copy size={12} />}
                            Copy Info
                        </button>
                    </div>
                    <div className="p-4 space-y-0 bg-white dark:bg-slate-900 text-sm">
                        <DisplayRow label="First Name" value={viewReq.requestData?.proprietor_details?.firstname} />
                        <DisplayRow label="Surname" value={viewReq.requestData?.proprietor_details?.surname} />
                        <DisplayRow label="Middle Name" value={viewReq.requestData?.proprietor_details?.middle_name} />
                        <DisplayRow label="NIN" value={viewReq.requestData?.proprietor_details?.nin} />
                        <DisplayRow label="Phone" value={viewReq.requestData?.proprietor_details?.phone} />
                        <DisplayRow label="Email" value={viewReq.requestData?.proprietor_details?.email} isEmail />
                        <DisplayRow label="Address" value={viewReq.requestData?.proprietor_details?.address} />
                        <DisplayRow label="State" value={viewReq.requestData?.proprietor_details?.state} />
                        <DisplayRow label="LGA" value={viewReq.requestData?.proprietor_details?.lga} />
                    </div>
                  </div>
              </div>
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-2xl border border-slate-200 dark:border-slate-700/50">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      {/* Passport */}
                      {viewReq.requestData?.documents?.passport_url && (
                          <a href={viewReq.requestData.documents.passport_url} target="_blank" className="block group">
                              <div className="bg-white dark:bg-slate-900 rounded-xl h-32 flex items-center justify-center border border-slate-200 dark:border-slate-700 group-hover:border-orange-400 dark:group-hover:border-orange-500 overflow-hidden relative transition-colors shadow-sm">
                                  <img src={viewReq.requestData.documents.passport_url} className="object-contain w-full h-full" alt="Passport" />
                              </div>
                              <span className="text-[10px] text-center block mt-2 font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest group-hover:text-orange-500 transition-colors">Passport</span>
                          </a>
                      )}
                      
                      {/* Signature */}
                      {viewReq.requestData?.documents?.signature_url && (
                          <a href={viewReq.requestData.documents.signature_url} target="_blank" className="block group">
                              <div className="bg-white dark:bg-slate-900 rounded-xl h-32 flex items-center justify-center border border-slate-200 dark:border-slate-700 group-hover:border-orange-400 dark:group-hover:border-orange-500 overflow-hidden relative transition-colors shadow-sm">
                                  <img src={viewReq.requestData.documents.signature_url} className="object-contain w-full h-full" alt="Signature" />
                              </div>
                              <span className="text-[10px] text-center block mt-2 font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest group-hover:text-orange-500 transition-colors">Signature</span>
                          </a>
                      )}

                      {/* NIN Slip */}
                      {viewReq.requestData?.documents?.nin_slip_url && (
                          <a href={viewReq.requestData.documents.nin_slip_url} target="_blank" className="block group">
                              <div className="bg-white dark:bg-slate-900 rounded-xl h-32 flex items-center justify-center border border-slate-200 dark:border-slate-700 group-hover:border-orange-400 dark:group-hover:border-orange-500 overflow-hidden relative transition-colors shadow-sm">
                                  <img src={viewReq.requestData.documents.nin_slip_url} className="object-cover w-full h-full opacity-80 group-hover:opacity-100" alt="NIN Slip" />
                              ...
                              <span className="text-[10px] text-center block mt-2 font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest group-hover:text-orange-500 transition-colors">NIN Slip</span>
                          </a>
                      )}
                  </div>
              </div>
              ...
              {actionType === 'PROCESSING' && (
                <div className="bg-orange-50 dark:bg-orange-900/20 p-4 rounded-xl border border-orange-100 dark:border-orange-800/50">
                  <p className="text-sm text-orange-800 dark:text-orange-300 font-medium leading-relaxed">
                    This will update the status to <span className="font-bold">"Processing"</span> so the agent knows you have started working on this CAC registration.
                  </p>
                </div>
              )}
              ...
              {actionType === 'APPROVE' && (
                <>
                  <div className="bg-green-50 dark:bg-green-900/10 p-4 rounded-xl border border-green-200 dark:border-green-800/30">
                    <label className="block text-[10px] font-bold text-green-800 dark:text-green-500 mb-2 uppercase tracking-widest">1. Upload Certificate (PDF/Image)</label>
                    <input 
                      type="file" 
                  ...
                  <div className="bg-blue-50 dark:bg-blue-900/10 p-4 rounded-xl border border-blue-200 dark:border-blue-800/30">
                    <label className="block text-[10px] font-bold text-blue-800 dark:text-blue-500 mb-2 uppercase tracking-widest">2. Upload Status Report (PDF/Image)</label>
                    <input 
                      type="file" 
                      onChange={e => setStatusFile(e.target.files?.[0] || null)}
                      className="block w-full text-sm text-slate-500 dark:text-slate-400 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-blue-600 file:text-white hover:file:bg-blue-700 cursor-pointer transition-colors"
                    />
                  </div>
                  <div>
