...
import { 
  ...
  Monitor, Code, Globe, User, MapPin, CreditCard, Layers, FileBadge
} from 'lucide-react';
...
export default function AdminBvnUserQueue() {
  ...
  const fetchQueue = async () => {
    ...
    try {
      ...
      if (res.data.status) {
          ...
          const uniqueRequests = sortedData.filter((item: any) => {
  ...
  useEffect(() => {
    ...
    // Source Filter
    if (sourceFilter !== 'ALL') {
      result = result.filter(r => getChannel(r.requestData?.reference || r.requestData?.clientReference || r.id) === sourceFilter);
    }
    ...
    if (searchQuery) {
      ...
      result = result.filter(r => 
          ...
          r.requestData?.parkway_wallet_id?.toLowerCase().includes(q) ||
  ...
  const handleActionSubmit = async () => {
    ...
    try {
      ...
      const note = actionType === 'REJECT' ? rejectionReason : (adminNote || 'Onboarding Successful');
  ...
  // Bulk Formatters
  const formatApplicantDetails = (item: any) => {
    const d = item.requestData || {};
    return `Name: ${d.first_name || ''} ${d.last_name || ''}; DOB: ${d.date_of_birth || 'N/A'}; Phone: ${d.phone_number || 'N/A'}; Email: ${d.email || 'N/A'}; NIN: ${d.nin || 'N/A'}; BVN: ${d.bvn || 'N/A'};`;
  };

  const formatBankingDetails = (item: any) => {
    const d = item.requestData || {};
    return `Parkway ID: ${d.parkway_wallet_id || 'N/A'}; Bank: ${d.bank_name || 'N/A'}; Account: ${d.account_number || 'N/A'}; Acc Name: ${d.account_name || 'N/A'};`;
  };

  const formatLocationDetails = (item: any) => {
    const d = item.requestData || {};
    return `Home: ${d.home_address || 'N/A'}; Agent Loc: ${d.agent_location || 'N/A'}; State: ${d.state_of_residence || 'N/A'}; LGA: ${d.local_government || 'N/A'}; Senatorial: ${d.senatorial_district || 'N/A'};`;
  };
      ...
      <div className="flex flex-col lg:flex-row gap-4 justify-between bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
        <div>
           <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
             <Smartphone className="w-7 h-7 text-teal-600 dark:text-teal-400" /> BVN User Enrollment
           </h1>
           <p className="text-gray-500 text-sm mt-1 font-medium">Manage NIBSS Agent Onboarding Requests.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-center">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input 
              type="text"
              placeholder="Search Name, NIN, Parkway ID..."
              value={searchQuery}
          ...
          <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
            <thead className="bg-slate-50 dark:bg-slate-950/50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider whitespace-nowrap">Parkway ID</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
              {filteredRequests.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center text-slate-500 dark:text-slate-400 font-medium">No onboarding requests found.</td>
                </tr>
              ) : (
                  ...
                  <tr key={req.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-[10px] text-slate-400 dark:text-slate-500 font-mono mt-0.5">
                        {req.requestData?.reference || req.requestData?.clientReference || req.id}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2 mb-1">
                        {getChannelBadge(req.requestData?.reference || req.requestData?.clientReference || req.id)}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="font-bold text-slate-800 dark:text-slate-200 uppercase text-xs tracking-wide max-w-[200px] truncate" title={`${req.requestData?.first_name} ${req.requestData?.last_name}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm font-mono text-teal-600 dark:text-teal-400 font-bold tracking-widest bg-teal-50 dark:bg-teal-900/30 px-2.5 py-1 rounded-lg border border-teal-200 dark:border-teal-800/50">
                        {req.requestData?.parkway_wallet_id || 'N/A'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-4xl relative my-8 border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200">
            ...
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 p-6 bg-slate-50 dark:bg-slate-950/50 rounded-t-3xl">
              <div>
                 <h3 className="text-xl font-black text-slate-900 dark:text-white flex items-center gap-2">
                   <FileBadge className="w-5 h-5 text-teal-500" /> Enrollment Details
                 </h3>
                 <p className="text-xs text-slate-500 dark:text-slate-400 font-mono mt-1">Ref: {viewReq.requestData?.reference || viewReq.requestData?.clientReference || viewReq.id}</p>
              </div>
            ...
            <div className="p-6 max-h-[70vh] overflow-y-auto space-y-6 custom-scrollbar">
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700/50 space-y-2.5">
                <h4 className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-3">Submitting Agent</h4>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Applicant Data */}
                  <div className="border border-teal-200 dark:border-teal-800/50 rounded-xl overflow-hidden shadow-sm shadow-teal-100/50 dark:shadow-none h-fit flex flex-col">
                    <div className="bg-teal-50 dark:bg-teal-900/10 p-3 border-b border-teal-200 dark:border-teal-800/50 flex justify-between items-center">
                        <span className="font-bold text-teal-900 dark:text-teal-500 text-sm">Applicant Info</span>
                        <button 
                            onClick={() => copyText(formatApplicantDetails(viewReq), 'app_details')}
                            className="text-[10px] bg-white dark:bg-slate-800 border border-teal-200 dark:border-teal-700 text-teal-700 dark:text-teal-500 font-bold px-2.5 py-1.5 rounded shadow-sm hover:bg-teal-50 dark:hover:bg-teal-900/30 flex items-center gap-1 uppercase tracking-wider transition-colors"
                        >
                            {copiedId === 'app_details' ? <Check size={12} className="text-green-500" /> : <Copy size={12} />}
                            Copy Info
                        </button>
                    </div>
                    <div className="p-4 space-y-0 bg-white dark:bg-slate-900 text-sm">
                        ...
                        <DisplayRow label="Last Name" value={viewReq.requestData?.last_name} />
                        <DisplayRow label="Date of Birth" value={viewReq.requestData?.date_of_birth} />
                        <DisplayRow label="Phone Number" value={viewReq.requestData?.phone_number} />
                        <DisplayRow label="Email" value={viewReq.requestData?.email} isEmail />
                        <DisplayRow label="BVN" value={viewReq.requestData?.bvn} />
                        <DisplayRow label="NIN" value={viewReq.requestData?.nin} />
                    </div>
                  </div>

                  {/* Banking & Location Data */}
                  <div className="space-y-6 flex flex-col h-full">
                      {/* Banking */}
                      <div className="border border-purple-100 dark:border-purple-800/30 rounded-xl overflow-hidden shadow-sm flex flex-col">
                        <div className="bg-purple-50 dark:bg-purple-900/10 p-3 border-b border-purple-100 dark:border-purple-800/30 flex justify-between items-center">
                            <span className="font-bold text-purple-900 dark:text-purple-400 text-sm">Banking & Wallet</span>
                            <button 
                                onClick={() => copyText(formatBankingDetails(viewReq), 'bank_details')}
                            ...
                            >
                                {copiedId === 'bank_details' ? <Check size={12} className="text-green-500" /> : <Copy size={12} />}
                                Copy Info
                            </button>
                        </div>
                        <div className="p-4 space-y-0 bg-white dark:bg-slate-900 text-sm">
                            <DisplayRow label="Parkway ID" value={viewReq.requestData?.parkway_wallet_id} />
                            <DisplayRow label="Bank Name" value={viewReq.requestData?.bank_name} />
                            <DisplayRow label="Account No" value={viewReq.requestData?.account_number} />
                            <DisplayRow label="Account Name" value={viewReq.requestData?.account_name} />
                        </div>
                      </div>

                      {/* Location */}
                      <div className="border border-slate-200 dark:border-slate-700/50 rounded-xl overflow-hidden shadow-sm flex flex-col">
                        <div className="bg-slate-50 dark:bg-slate-800/50 p-3 border-b border-slate-200 dark:border-slate-700/50 flex justify-between items-center">
                            <span className="font-bold text-slate-700 dark:text-slate-300 text-sm">Location Details</span>
                            <button 
                                onClick={() => copyText(formatLocationDetails(viewReq), 'loc_details')}
                                className="text-[10px] bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 font-bold px-2.5 py-1.5 rounded shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 flex items-center gap-1 uppercase tracking-wider transition-colors"
                            >
                                {copiedId === 'loc_details' ? <Check size={12} className="text-green-500" /> : <Copy size={12} />}
                                Copy Info
                            </button>
                        </div>
                        <div className="p-4 space-y-0 bg-white dark:bg-slate-900 text-sm">
                            <DisplayRow label="Home Address" value={viewReq.requestData?.home_address} />
                            <DisplayRow label="Agent Location" value={viewReq.requestData?.agent_location} />
                            <DisplayRow label="State" value={viewReq.requestData?.state_of_residence} />
                            <DisplayRow label="LGA" value={viewReq.requestData?.local_government} />
                            <DisplayRow label="Senatorial" value={viewReq.requestData?.senatorial_district} />
                        </div>
                      </div>
                  </div>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-md p-6 relative my-8 border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200">
            ...
            <div className="flex justify-between items-start mb-6 border-b border-slate-100 dark:border-slate-800 pb-4">
              <div>
                 ...
                 <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 font-mono">Ref: {selectedReq.requestData?.reference || selectedReq.requestData?.clientReference || selectedReq.id}</p>
              </div>
              ...
              {actionType === 'PROCESSING' && (
                <div className="bg-teal-50 dark:bg-teal-900/20 p-4 rounded-xl border border-teal-100 dark:border-teal-800/50">
                  <p className="text-sm text-teal-800 dark:text-teal-300 font-medium leading-relaxed">
                    This will update the status to <span className="font-bold">"Processing"</span> so the agent knows you have started working on this enrollment request.
                  </p>
                </div>
              )}
              ...
              {actionType === 'APPROVE' && (
                <>
                  <div className="bg-green-50 dark:bg-green-900/10 p-4 rounded-xl border border-green-200 dark:border-green-800/30">
                    <label className="block text-[10px] font-bold text-green-800 dark:text-green-500 mb-2 uppercase tracking-widest">Upload Confirmation Document (Optional)</label>
                    <input 
                      type="file" 
                  ...
                  <div>
                    ...
                    <textarea
                      placeholder="E.g. Onboarding successful..."
              ...
              {actionType === 'REJECT' && (
                <>
                  <div>
                    ...
                    <textarea
                      placeholder="Why is this enrollment being declined?"
