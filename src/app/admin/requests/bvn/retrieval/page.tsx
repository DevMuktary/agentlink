...
import { 
  ...
  Globe, Code, Copy, Check, ImageIcon, Eye, FileBadge, X
} from 'lucide-react';
...
export default function AdminBvnRetrievalQueue() {
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(r => 
          r.requestData?.phoneNumber?.includes(q) ||
          r.requestData?.phone?.includes(q) ||
          r.requestData?.name?.toLowerCase().includes(q) ||
  ...
  const handleActionSubmit = async () => {
    ...
    if (actionType === 'REJECT' && !rejectionReason) return alert("Enter a rejection reason.");
    ...
    try {
      ...
      if (actionType === 'APPROVE') {
          ...
          formData.append('note', actionType === 'REJECT' ? rejectionReason : (adminNote || 'Processing Request'));
  ...
  const getTypeBadge = (type: string) => {
      if (type.includes('PHONE')) return <span className="bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-800/50 px-2 py-1 rounded text-[9px] font-bold uppercase tracking-wider whitespace-nowrap">Phone Retrieval</span>;
      if (type.includes('CRM')) return <span className="bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400 border border-purple-200 dark:border-purple-800/50 px-2 py-1 rounded text-[9px] font-bold uppercase tracking-wider whitespace-nowrap">CRM Search</span>;
      return <span className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 px-2 py-1 rounded text-[9px] font-bold uppercase tracking-wider whitespace-nowrap">Standard Retrieval</span>;
  };

  const getServiceName = (type: string) => {
    if (type.includes('PHONE')) return 'Phone Number Retrieval';
    if (type.includes('CRM')) return 'CRM Screenshot Search';
    return 'Standard BVN Retrieval';
  };
  ...
  const formatSearchCriteria = (item: any) => {
    const data = item.requestData || {};
    let criteria = [];
    if (data.phoneNumber || data.phone) criteria.push(`Phone: ${data.phoneNumber || data.phone}`);
    if (data.name) criteria.push(`Name: ${data.name}`);
    return criteria.length > 0 ? criteria.join('; ') + ';' : 'N/A';
  };
      ...
      <div className="flex flex-col lg:flex-row gap-4 justify-between bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
        <div>
           <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
             <Search className="w-7 h-7 text-indigo-600 dark:text-indigo-400" /> BVN Retrieval
           </h1>
           <p className="text-gray-500 text-sm mt-1 font-medium">Process lost BVN recovery requests.</p>
        </div>
          ...
          <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
            <thead className="bg-slate-50 dark:bg-slate-950/50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider whitespace-nowrap">Search Criteria</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
              {filteredRequests.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-slate-500 dark:text-slate-400 font-medium">No retrieval requests found.</td>
                </tr>
              ) : (
                filteredRequests.map((req) => (
                  <tr key={req.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-900 dark:text-white font-mono font-bold whitespace-nowrap">
                      {req.requestData?.phoneNumber || req.requestData?.phone || req.requestData?.name || '-'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border whitespace-nowrap ${getStatusColor(req.status)}`}>
                        {req.status === 'COMPLETED' ? 'Found' : req.status === 'FAILED' ? 'Not Found' : req.status}
                      </span>
                    </td>
            ...
            <div className="p-6 max-h-[70vh] overflow-y-auto space-y-6 custom-scrollbar">
              ...
              <div className="border border-indigo-100 dark:border-indigo-800/30 rounded-xl overflow-hidden shadow-sm">
                 <div className="bg-indigo-50 dark:bg-indigo-900/10 p-3 border-b border-indigo-100 dark:border-indigo-800/30 flex justify-between items-center">
                    <span className="font-bold text-indigo-900 dark:text-indigo-400 text-sm">Search Criteria</span>
                    <button 
                        onClick={() => copyText(formatSearchCriteria(viewReq), 'search_criteria')}
                    ...
                    >
                        {copiedId === 'search_criteria' ? <Check size={12} className="text-green-500" /> : <Copy size={12} />}
                        Copy Search Data
                    </button>
                 </div>
                 <div className="p-4 space-y-0 bg-white dark:bg-slate-900 text-sm">
                    ...
                    {Object.entries(viewReq.requestData || {}).map(([key, value]) => {
                        // Skip internal tracking keys and screenshot URLs from text loop
                        if (!value || typeof value === 'object' || ['service_code', 'clientReference', 'reference', 'derivedFrom', 'screenshotUrl'].includes(key)) return null;
                 ...
                 {viewReq.requestData?.screenshotUrl && (
                     <div className="p-4 bg-white dark:bg-slate-900 border-t border-indigo-50 dark:border-indigo-800/30">
                         <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase font-bold mb-2 block tracking-widest flex items-center gap-1.5">
                             <ImageIcon size={14} /> CRM Proof Screenshot
                         </span>
                         <a href={viewReq.requestData.screenshotUrl} target="_blank" className="block w-full h-40 bg-slate-100 dark:bg-slate-800 rounded-xl border border-indigo-200 dark:border-indigo-800/50 overflow-hidden relative group shadow-sm">
                             <img src={viewReq.requestData.screenshotUrl} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition duration-300" />
              ...
              {actionType === 'PROCESSING' && (
                <div className="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800/50">
                  <p className="text-sm text-indigo-800 dark:text-indigo-300 font-medium leading-relaxed">
                    This will update the status to <span className="font-bold">"Processing"</span> so the agent knows you are currently searching for this BVN.
                  </p>
                </div>
              )}
              ...
              {actionType === 'APPROVE' && (
                <>
                  <div className="bg-green-50 dark:bg-green-900/10 p-4 rounded-xl border border-green-200 dark:border-green-800/30">
                    <label className="block text-[10px] font-bold text-green-800 dark:text-green-500 mb-2 uppercase tracking-widest">Recovered BVN (Required)</label>
                    <div className="flex items-center border border-slate-300 dark:border-slate-600 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-green-500 transition-all bg-white dark:bg-slate-900 shadow-sm mb-4">
                        ...
                        <input 
                            ...
                            className="w-full p-3 text-sm outline-none font-mono tracking-widest font-bold text-slate-900 dark:text-white bg-transparent"
                            placeholder="Enter 11-digit BVN" 
                        />
                    </div>
                    
                    <label className="block text-[10px] font-bold text-green-800 dark:text-green-500 mb-2 uppercase tracking-widest">Or Upload Slip (Optional)</label>
                    <input 
                      type="file" 
                      onChange={e => setResultFile(e.target.files?.[0] || null)}
                      className="block w-full text-sm text-slate-500 dark:text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-green-600 file:text-white hover:file:bg-green-700 cursor-pointer transition-colors"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      placeholder="E.g. Retrieval successful..."
              ...
              {actionType === 'REJECT' && (
                <>
                  <div>
                    ...
                    <textarea
                      placeholder="Why is this retrieval being declined? (e.g., Not Found)"
